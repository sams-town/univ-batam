...
const dummyPenugasan: Penugasan[] = [
  {
    ...
    jabatan: 'Dosen',
    tugas: 'Persiapan Ujian Tengah Semester',
    deadline: '2024-02-15',
    status: 'Dikerjakan'
  },
  {
              ...
              {data.length > 0 ? (
                data.map((item) => (
                  <TableRow key={item.id} className="border-slate-200 hover:bg-slate-50">
                    ...
                    <TableCell>
                      <span className={`px-2 py-1 rounded-full text-xs ${item.status === 'Selesai' ? 'bg-green-100 text-green-700' : item.status === 'Dikerjakan' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        {item.status}
                      </span>
                    </TableCell>
                    <TableCell>
