...
const dummyLaporanKinerja: LaporanKinerja[] = [
  { id: '1', nama: 'Dr. Budi Santoso', jabatan: 'Dosen', periode: 'Triwulan 1 2024', nilai: '92', status: 'Sangat Baik' },
  { id: '2', nama: 'Siti Aminah', jabatan: 'Staff Administrasi', periode: 'Triwulan 1 2024', nilai: '85', status: 'Baik' }
]
...
export default function LaporanKinerjaPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<LaporanKinerja | null>(null)
  const [formData, setFormData] = useState({ nama: '', jabatan: '', periode: '', nilai: '', status: '' })
  ...
  const handleAdd = () => {
    const newItem: LaporanKinerja = { id: Date.now().toString(), ...formData }
    setData([...data, newItem])
    setIsModalOpen(false)
    resetForm()
  }

  const handleEdit = (item: LaporanKinerja) => {
  ...
  const resetForm = () => {
    setFormData({ nama: '', jabatan: '', periode: '', nilai: '', status: '' })
  }
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogTrigger asChild>
            <Button className="flex items-center gap-2" onClick={() => { setSelectedItem(null); resetForm() }}>
              <Plus className="h-4 w-4" /> Tambah Laporan Kinerja
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold">{selectedItem ? 'Edit Laporan Kinerja' : 'Tambah Laporan Kinerja'}</DialogTitle>
              <DialogDescription className="text-slate-500">SISTEM MANAJEMEN LAPORAN KINERJA</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="periode" placeholder="Triwulan 1 2024" className="w-full bg-slate-50 border-slate-200" value={formData.periode} onChange={(e) => setFormData(prev => ({ ...prev, periode: e.target.value }))} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="nilai" className="text-slate-500 font-medium">NILAI</Label>
                  <Input id="nilai" type="number" placeholder="92" className="w-full bg-slate-50 border-slate-200" value={formData.nilai} onChange={(e) => setFormData(prev => ({ ...prev, nilai: e.target.value }))} />
                </div>
                <div className="space-y-2">
      ...
      <Card className="border-slate-200 bg-white">
        <CardHeader className="pb-4">
          <CardTitle className="flex items-center gap-2 text-xl font-bold">
            <FileCheck className="h-5 w-5 text-blue-500" /> Daftar Laporan Kinerja
          </CardTitle>
            ...
            <TableBody>
              {data.length > 0 ? data.map(item => (
                <TableRow key={item.id} className="border-slate-200 hover:bg-slate-50">
                  ...
                  <TableCell>
                    <span className={`px-2 py-1 rounded-full text-xs ${item.status === 'Sangat Baik' ? 'bg-green-100 text-green-700' : item.status === 'Baik' ? 'bg-blue-100 text-blue-700' : item.status === 'Cukup' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>{item.status}</span>
                  </TableCell>
                  <TableCell>
